  ...
  return http.createServer(async (request, response) => {
    ...
    try {
      ...
      if (request.method === 'GET' && requestUrl.pathname === '/api/config') {
        sendJson(response, 200, publicConfiguration(environment, maxBodyBytes));
        return;
      }
...
async function loadBundledSample() {
  ...
  return {
    ...
    description: 'A self-authored staged-diff example with an exact-threshold defect and missing boundary test.',
...
function publicConfiguration(environment, maxBodyBytes) {
  const apiKey = serverApiKey(environment);
  return {
    defaultMode: 'offline',
